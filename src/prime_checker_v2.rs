...
fn check_one_number(num: i128) {
    //! Checks exactly one number for primeness; mention if it is Anti-Prime.
    let result: bool;
    ...
    if result == false {
        ...
        //// Debugging code; comment out for prod.
        // let _debug_statement = format!("Lower numbers with a higher or equal number of factors:\t{:?}", previous_highers);
        // println!("{}", _debug_statement.dimmed());
        if previous_highers.len() == 0 {
            let pre_anti_prime_statement = format!("{num} is not just {_not} a prime number, it is an {anti_prime} number; it's factors are:", num=num, _not="NOT".bold(), anti_prime="ANTI-PRIME".green().bold());
            let anti_prime = format!(
                "{pre_anti_prime_statement} {factors:?}",
                pre_anti_prime_statement = pre_anti_prime_statement,
                factors = factors
            );
            println!("{}", anti_prime);
        } else {
            println!(
                "{num} is not a prime number; it's factors are: {factors:?}",
                num = num,
                factors = factors
            );
        }
        ...
        println!(
            "{num} is a prime number; its only factors are {factors:?}.",
            num = num,
            factors = factors
        )
    }
...
pub fn run(option: i128, _num: i128) {
    ...
    if option == 1 {
        let disp_choice_1 = format!(
            "Option #{} chosen; checking if |{}| is a prime number.",
            option, _num
        );
        println!("{}", disp_choice_1.dimmed());
        check_one_number(num);
    } else if option == 2 {
        let disp_choice_2 = format!(
            "Option #{} chosen; finding all prime numbers until |{}|.",
            option, _num
        );
        println!("{}", disp_choice_2.dimmed());
        ...
        let disp_primes = format!(
            "The list of prime numbers til {} is {:?}.",
            num.abs(),
            primes
        );
        println!("{}", disp_primes.bright_green().bold());
    } else {
